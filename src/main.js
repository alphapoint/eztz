  ...
  (contract = {
    hash: function(operationHash, ind) {
      ...
      return utility.b58cencode(
        library.sodium.crypto_generichash(20, new Uint8Array(tt)),
        prefix.KT
      );
    },
    originate: function(
...
(tezos = {
  forge: function(head, opOb, validateLocalForge) {
    ...
    return node
      ...
      .then(function(remoteForgedBytes) {
        var localForgedBytes;
        localForgedBytes = utility.buf2hex(
...
function forgeOp(op) {
  ...
  switch (forgeOpTags[op.kind]) {
    ...
    case 4:
      fop += eztz.utility.buf2hex(
        eztz.utility.b58cdecode(op.pkh, eztz.prefix.tz1)
      );
      fop += op.secret;
    ...
    case 6:
      ...
      if (forgeOpTags[op.kind] == 5) {
        ...
        fop += eztz.utility.buf2hex(
          eztz.utility.b58cdecode(op.proposal, eztz.prefix.P)
        );
...
function forgePublicKeyHash(pkh) {
  ...
  fpkh = "0" + (t - 1).toString();
  fpkh += eztz.utility.buf2hex(
    eztz.utility.b58cdecode(pkh, eztz.prefix[pkh.substr(0, 3)])
  );
  return fpkh;
}
function forgePublicKey(pk) {
  ...
  fpk += eztz.utility.buf2hex(
    eztz.utility.b58cdecode(pk, eztz.prefix[pk.substr(0, 4)])
  );
  return fpk;
}
