...
import { derivePath, getKeyPair, getMasterKeyFromSeed } from '../src/ed25519hd';
...
const vector2Seed = 'fffcf9f6f3f0edeae7e4e1dedbd8d5d2cfccc9c6c3c0bdbab7b4b1aeaba8a5a29f9c999693908d8a8784817e7b7875726f6c696663605d5a5754514e4b484542';
const vector2 = [
  {
    path: "m/0'",
...
describe('Test vectors', () => {
  ...
  describe('Vector 1', () => {
    it('should have valid keys for vector 1 seed hex', () => {
      const key = Buffer.from(getMasterKeyFromSeed(vector1Seed));
    ...
    for (const vector of vector1) {
      it(`should valid for ${vector.path}`, async () => {
        const { privateKey, chainCode } = await derivePath(vector.path, vector1Seed);
        expect({
          path: vector.path,
          key: utility.buf2hex(privateKey),
          chainCode: utility.buf2hex(chainCode),
          publicKey: utility.buf2hex((await getKeyPair(privateKey)).publicKey)
        }).toEqual(vector);
      });
    }
  });
  ...
  describe('Vector 2', () => {
    it('should have valid keys for vector 2 seed hex', () => {
      const key = Buffer.from(getMasterKeyFromSeed(vector2Seed));
      const privateKey = key.slice(0, 32);
      const chainCode = key.slice(32);
      expect(privateKey.toString('hex')).toEqual('171cb88b1b3c1db25add599712e36245d75bc65a1a5c9e18d76f9f2b1eab4012');
      expect(chainCode.toString('hex')).toEqual('ef70a74db9c3a5af931b5fe73ed8e1a53464133654fd55e7a66f8570b8e33c3b');
    });
    for (const vector of vector2) {
      it(`should valid for ${vector.path}`, async () => {
        const { privateKey, chainCode } = await derivePath(vector.path, vector2Seed);
        expect({
          path: vector.path,
          key: utility.buf2hex(privateKey),
          chainCode: utility.buf2hex(chainCode),
